...
import org.kopi.ebics.client.TransferState;
 ...
 *
 */
public class FileTransferV5 {

  /**
   * Constructs a new FileTransfer session
   * @param session the user session
   */
  public FileTransferV5(EbicsSession session) {
   ...
   */
  public void sendFile(byte[] content, EbicsUploadOrder orderType)
    throws IOException, EbicsException
  {

    EbicsAdminOrderType ebicsAdminOrderType = orderType.getAdminOrderType();
    ...
    InitializationResponseElement response = new InitializationResponseElement(sender.getResponseBody(),
                                             ebicsAdminOrderType,
	                                         DefaultEbicsRootElement.generateName(ebicsAdminOrderType));
    response.build();
    session.getConfiguration().getTraceManager().trace(response);

    org.kopi.ebics.client.TransferState state = new org.kopi.ebics.client.TransferState(initializer.getSegmentNumber(), response.getTransactionId());

        while (state.hasNext()) {
            int segmentNumber = state.next();
            sendFile(initializer.getContent(segmentNumber), segmentNumber, state.isLastSegment(),
                state.getTransactionId(), ebicsAdminOrderType);
        }
    }
...
//   * @param end optional end of fetch term
    ...
    throws IOException, EbicsException
  {
    ...
    int					httpCode;
    org.kopi.ebics.client.TransferState state;
    Joiner				joiner;
   ...
   * @throws EbicsException server generated error
   */
  public void fetchFile(EbicsAdminOrderType orderType,
                        int segmentNumber,
                        boolean lastSegment,
                        byte[] transactionId,
                        Joiner joiner)
